...
//$f3->set('message', '');
    ...
    //echo $f3->get('AUTOLOAD');
            ...
            $this->commentAry = array('comment' => 'Ello');
            $this->eventInfo  = array('numberOfNames' => 1, 'eventId' => 99999, 'eventName' => 'Test Event', 'eventDate' =>  '2016-12-07',
            'eventContactEmail' => 'laurie@lyates.com', 'eventType' => 'event', 'eventLimit' => 2, 'active' => 'Y', );
     ...
     */
            ...
            switch ($testnum) {
               ...
               //	krumo($attendResp);
                     ...
                     $test->expect(
                         ($attendResp['added'] == 1),
                         'Test 2:- added = 1 in array from addAttendees for 1 person received '.$attendResp['added']
                     );
                    $this->db->exec('delete from events where eventId = :id', array(':id' => 0));     //Needed to allow it to be repeatable as I don't create the event in a direct call to addAttendees
                    break;
                case 3:
            ...
            */
               //	krumo(array(':id' => $this->eventInfo['eventId']));
                    ...
                    $this->db->exec('delete from attendees where eventId = :id', array(':id' => $this->eventInfo['eventId']));
                   //break;
                    $bodyAll = array('eventInfo' => $this->eventInfo, 'persons' => $this->persons, 'comment' => $this->commentAry);
               ...
               //	krumo($testResp);
                ...
                case 4: //  Ensure event limit is 2
                 ...
                 /*********  The event has been created it should have the correct eventId ******/
                     $test->expect(
                         $testResp->eventId == $this->eventInfo['eventId'],
           ...
           //      $testResp = $this->myiMock('addAttend', NULL, NULL, $bodyAllJson);
                    ...
                    $options = array(    'method'  => 'POST',    'content' => $bodyAllJson);
           //      $testResp =\Web::instance()->request($url, $options); //('POST /addAttend [sync]', NULL, NULL, $bodyAllJson);
                 ...
                 //krumo($testResp);
                    ...
                    $this->event->load(array('eventId =?', $this->eventInfo['eventId']));
                     $test->expect(
                         $this->event->eventCurrentCount == 3,
           ...
           //      $attendeesCount = $this->attendee->count(array('requestStatus = "Waitlisted" and eventId =?', $this->eventInfo['eventId']));
                 ...
                 //	krumo($attendeesCount);
                     ...
                     $test->expect(
                         $attendeesCount == 3,
                         'Test 4g:- POST to addAttend expect only 3 attendees, received '.$attendeesCount
                     )   ;
               ...
               //	krumo($bodyAllJson);
                    ...
                    $attendeesCount = $this->attendee->count();
                     $test->expect(
                         $attendeesCount == 3,
                         'Test 4i:- POST to addAttend expect only 3 attendees, received '.$attendeesCount
                     )   ;
               //	krumo("4J");
               ...
               //krumo("4L");
                    ...
                    $this->event->load(array('eventId =?', $this->eventInfo['eventId']));
                     $test->expect(
                         $this->event->eventCurrentCount == 5,
                         'Test 4m:- POST to addAttend expect eventCurrentCount == 5 ,received '.$this->event->eventCurrentCount
                     )   ;

                    $waitlistedCount = $this->attendee->count(array('requestStatus = "Waitlisted" and eventId =?', $this->eventInfo['eventId']));
                     $test->expect(
                         $waitlistedCount == 2,
                         'Test 4n:- POST to addAttend expect count attendees as Waitlisted to be 2, received '.$waitlistedCount
                     )   ;
                ...
                case 5:
                    ...
                    $testResp = $this->event->add($this->eventInfo);
                    $this->commentAry = array('comment' => "ONE");
                    $bodyAll = array('eventInfo' => $this->eventInfo, 'persons' => $this->persons, 'comment' => $this->commentAry);
           ...
           //krumo("5A");  die();
                    $testResp = $this->myiMock('addAttend', null, null, $bodyAllJson);
                     ...
                     $this->event->load(array('eventId =?', $this->eventInfo['eventId']));
                     $test->expect(
                         $this->event->eventCurrentCount == 1,
               ...
               //krumo("5B");	die();
                    ...
                    $this->persons[] = $this->attendee2;
                    $this->eventInfo['numberOfNames'] = 2;
                    $this->commentAry = array('comment' => " TWO");
                    $bodyAll = array('eventInfo' => $this->eventInfo, 'persons' => $this->persons, 'comment' => $this->commentAry);
                    $bodyAllJson = json_encode($bodyAll);
               //krumo("5A");
                    $testResp = $this->myiMock('addAttend', null, null, $bodyAllJson);
                     ...
                     $this->event->load(array('eventId =?', $this->eventInfo['eventId']));
                     $test->expect(
                         $this->event->eventCurrentCount == 2,
                         'Test 5f:- POST to addAttend expect eventCurrentCount == 2 ,received '.$this->event->eventCurrentCount
                     )   ;
                     $attendeeCount = $this->attendee->count(array('requestStatus ="Booked" and eventId =?', $this->eventInfo['eventId']));
                     $test->expect(
                         $attendeeCount == 2,
                         'Test 5g:- POST to addAttend expect Booked count  == 2 ,received '.$attendeeCount
                     )   ;
                     ...
                     $test->expect(
                         $this->attendee->requestComment == 'ONE/ TWO',
                ...
                case 6:
                       ...
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Booked" and eventId =?', $this->eventInfo['eventId']));
                        $test->expect(
                            $attendeeCount == 2,
                            'Test 6a:- POST to addAttend expect Booked count  == 2 ,received '.$attendeeCount
                        )   ;
                       $this->persons = array();
                       $this->persons[] = $this->attendee4;
                       $this->persons[] = $this->attendee3;
                       ...
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Booked" and eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $attendeeCount == 2,
                               'Test 6b:- POST to addAttend expect Booked count  == 2 ,received '.$attendeeCount
                           )   ;
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Waitlisted" and eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $attendeeCount == 2,
                               'Test 6c:- POST to addAttend expect Waitlisted count  == 2 ,received '.$attendeeCount
                           )   ;
                       $this->event->load(array('eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $this->event->eventCurrentCount == 4,
                       ...
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Booked" and eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $attendeeCount == 2,
                               'Test 6e:- POST to addAttend expect Booked count  == 2 ,received '.$attendeeCount
                           )   ;
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Waitlisted" and eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $attendeeCount == 2,
                               'Test 6f:- POST to addAttend expect Waitlisted count  == 2 ,received '.$attendeeCount
                           )   ;
                       $this->event->load(array('eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $this->event->eventCurrentCount == 4,
                     ...
                     //	krumo("6H");
                       ...
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Booked" and eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $attendeeCount == 2,
                               'Test 6h:- POST to addAttend expect Booked count  == 2 ,received '.$attendeeCount
                           )   ;
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Waitlisted" and eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $attendeeCount == 3,
                               'Test 6i:- POST to addAttend expect Waitlisted count  == 3 ,received '.$attendeeCount
                           )   ;
                       $this->event->load(array('eventId =?', $this->eventInfo['eventId']));
                           $test->expect(
                               $this->event->eventCurrentCount == 5,
                ...
                case 7:
                    ...
                    $attendeeCount = $this->attendee->count(array('requestStatus ="Booked" and eventId =?', $this->eventInfo['eventId']));
                        $test->expect(
                            $attendeeCount == 2,
                            'Test 7a:- POST to addAttend expect Booked count  == 2 ,received '.$attendeeCount
                        )   ;
                     //	krumo($testResp);
                       $test->expect(
                           $testResp == 'Booked',
                       ...
                       $attendeeCount = $this->attendee->count(array('requestStatus ="Booked" and eventId =?', $this->eventInfo['eventId']));
                        $test->expect(
                            $attendeeCount == 3,
                            'Test 7c:- POST to addAttend expect Booked count  == 3 ,received '.$attendeeCount
                        )   ;
                     //	krumo($testResp);
                       $test->expect(
                           $testResp == 'Booked',
...
*/
 ...
 /******  various test     public functions **/
    ...
    //$eventInfo = array('eventId' => 1001);
            $eventInfo = array('eventId' =>  2574, 'eventName' => 'fiddler-on-the-roof-the-musical-at-the-salon-theatre-in-fuengirola',
            'eventDate' => '2016-11-29', 'eventType' => 'event', 'eventLimit' => 55, 'eventCurrentCount' => 11    , 'eventContactEmail' => 'laurie29.lyates@gmail.com', 'active' => 'Y', );
            krumo($eventInfo);
            $bodyAll = array('eventInfo' => $eventInfo);
            $bodyAllJson = json_encode($bodyAll);
            $options = array(
            'method'  => 'POST',
 ...
 /******  various test     public functions **/
    ...
    //krumo("Event ".$eventInfo['eventDate']);
        ...
        //return 0;
            ...
            $event->erase();
            $eventInfo = array('eventId' =>  2574, 'eventName' => 'fiddler-on-the-roof-the-musical-at-the-salon-theatre-in-fuengirola',
            'eventDate' => '2016-10-09', 'eventType' => 'event', 'eventLimit' => 55, 'eventCurrentCount' => 9, 'eventContactEmail' => 'laurie9.lyates@gmail.com', 'active' => 'Y', );
            $eventInfoJson = json_encode($eventInfo);
    ...
    /*********  Now test change of date to 10th *****/
            $eventInfo = array('eventId' =>  2574, 'eventName' => 'fiddler-on-the-roof-the-musical-at-the-salon-theatre-in-fuengirola',
            'eventDate' => '2016-10-10', 'eventType' => 'event', 'eventLimit' => 55, 'eventCurrentCount' => 10    , 'eventContactEmail' => 'laurie10.lyates@gmail.com', 'active' => 'Y', );
            $event->reset();
            $eventInfoJson = json_encode($eventInfo);
            //krumo(" Event ".$eventInfo['eventDate']);
            $this->addEvent($eventInfoJson);
        ...
        /*********  Now test change of date to 23rd *****/
            $eventInfo = array('eventId' =>  2574, 'eventName' => 'fiddler-on-the-roof-the-musical-at-the-salon-theatre-in-fuengirola',
            'eventDate' => '2016-10-29', 'eventType' => 'event', 'eventLimit' => 55, 'eventCurrentCount' => 29    , 'eventContactEmail' => 'laurie29.lyates@gmail.com', 'active' => 'Y', );
    //krumo("Event ".$eventInfo['eventDate']);
            $event->reset();
            $eventInfoJson = json_encode($eventInfo);
            $this->addEvent($eventInfoJson);
 ...
 //test of remote delete
  ...
  "eventdata" : [    {"id" : "1", "eventName":"cell11", "eventDate" :"cell12", "eventContactEmail" :"cell13"},
    {"id" : "2", "eventName":"cell21", "eventDate" :"cell22", "eventContactEmail" :"cell23"} ]}';
            ...
            $eventCount = $this->event->count(array('active = "Y"'));
            krumo($eventCount);
            ...
            foreach ($events as $eventnum => $event) {
                //krumo($event);
                $eventArray['eventdata'][] = array('id' => $event->id, 'eventName' => $event->eventName, 'eventDate' => $event->eventDate, 'eventContactEmail' => $event-> eventContactEmail,
                    'eventLimit' => $event->eventLimit, 'eventCurrentCount' => $event->eventCurrentCount, 'eventFull' => $event->eventFull, );
